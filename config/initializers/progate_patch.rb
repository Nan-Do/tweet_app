module RedirectPatch
  def redirect_to(options = {}, response_status = {})
    if options.is_a?(String)
      super(options + '?instanceId=65b42d2cc2e3&containerPort=3000&languageName=rails5&locale=en', response_status)
    else
  ...
  def stylesheet_link_tag(*sources)
    ...
    sources.uniq.map { |source|
      tag_options = {
        "rel" => "stylesheet",
        "media" => "screen",
        "href" => path_to_stylesheet(source, path_options) + '&instanceId=65b42d2cc2e3&containerPort=3000&languageName=rails5&locale=en'
      }.merge!(options)
